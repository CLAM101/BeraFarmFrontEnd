...
export class GameServiceService {
  ...
  async getCubBalance(beraCubContract: any): Promise<number> {
    const cubBalance = await beraCubContract.balanceOf(
  ...
  async getFuzzBalance(fuzzTokenContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(
        await fuzzTokenContract.balanceOf(window.ethereum.selectedAddress)
      )
    );
  }
  ...
  async getTotalBurned(fuzzTokenContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(await fuzzTokenContract.totalBurned())
    );
  }

  async getEmissionsPerCub(beraFarmContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(await beraFarmContract.currentDailyRewards())
    );
  }
  ...
  async getUnclaimedRewards(beraFarmContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(
        await beraFarmContract.getTotalClaimable(
          window.ethereum.selectedAddress
        )
      )
    );
  ...
  async getPlayerCompoundCost(beraFarmContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(
        await beraFarmContract.getWalletCompoundCost(
          window.ethereum.selectedAddress
        )
      )
    );
  ...
  async getFuzzPrice(beraFarmContract: any): Promise<number> {
    return parseFloat(
      ethers.formatEther(await beraFarmContract.getFuzzPrice())
    );
  }
}
